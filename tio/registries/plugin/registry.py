from ..registry import MetadataRegistry


class PluginRegistry(MetadataRegistry):
    """
    Internal registry that discovers and manages plugin classes in Tio.

    Scans modules within the `tio.family` package for classes implementing
    Transforms, Inputs, Outputs, and Runners, and registers them as objects
    for internal use. This allows job manifests to reference plugins without
    having to manage instantiation or discovery.

    The PluginRegistry is used internally by Tio and is not available in the
    Context for custom manipulation.
    """

    def __init__(self) -> None:
        super().__init__()
