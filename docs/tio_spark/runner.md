...
## Passing Spark config options

Any key not listed above is forwarded to `SparkSession.builder.config()`.
...
Use `jars_packages` to add Maven-coordinated dependencies to the Spark classpath. This is equivalent to setting `spark.jars.packages`. A single string is accepted in addition to a list.
...
Set `streaming: true` to enable streaming execution. When streaming is enabled, [SparkFileInput](file-input.md) uses `spark.readStream` instead of `spark.read`, and each input must point to exactly one directory path.
