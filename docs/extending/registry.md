...
Every plugin (Input, Transform, Output) runs inside a job execution. During that execution, Tiozin makes all configured registries available through `self.context.registries`. This gives your plugin direct access to secrets, schemas, batches, and the other infrastructure services your job depends on, without passing anything through the constructor.
